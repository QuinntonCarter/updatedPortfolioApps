...
export default function Comment({
  ...
  const { deleteComment, appError } = useContext(AppContext);
  ...
  async function validateDeletion(postId, commId) {
    if (userIsAuthor) {
      ...
      return setAppError({
        status: "",
        msg: "Error: cannot delete, this isn't your comment",
      });
    }
  }

  const canDeleteComment = userIsAuthor && (
    <button onClick={() => validateDeletion(postId, _id)} className="deleteBtn">
      {" "}
      Delete Comment{" "}
    </button>
  );
