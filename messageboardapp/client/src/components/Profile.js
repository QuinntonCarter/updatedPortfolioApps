...
export default function Profile() {
  ...
  const userPostsMapped =
    posts.userPosts &&
    posts.userPosts.map((posts) => (
      <div className="profilePosts">
        <Posts {...posts} _id={posts._id} userId={posts.user} key={posts._id} />
      </div>
    ));
  ...
  return (
    <div>
      ...
      <h3> posts: </h3>
      <section className="postsGrid">{userPostsMapped}</section>
      <h3> comments: </h3>
      <section className="commentsGrid">{userCommentsMapped}</section>
    </div>
  );
}
